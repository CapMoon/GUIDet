...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, res_conv=False, act_layer=nn.ReLU, groups=1,
        ...
        self.conv1 = ODConv2d(in_channels, c, kernel_size=1, stride=1, padding=0)  # [64, 256, 1, 1]
        self.bn1 = norm_layer(c)
        self.act1 = act_layer(inplace=True)

        self.conv2 = ODConv2d(c, c, kernel_size=3, stride=stride, groups=groups, padding=1)
        self.bn2 = norm_layer(c)
        self.act2 = act_layer(inplace=True)

        self.conv3 = ODConv2d(c, out_channels, kernel_size=1, stride=1, padding=0)
        self.bn3 = norm_layer(out_channels)
        self.act3 = act_layer(inplace=True)

        if res_conv:
            self.residual_conv = ODConv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,
        ...
        self.fc1 = ODConv2d(in_features, hidden_features, 1)
        self.act = act_layer()
        self.fc2 = ODConv2d(hidden_features, out_features, 1)
...
class LVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_codes, channel_ratio=0.25, base_channel=64):
        ...
        self.LVC = nn.Sequential(
            ...
            ODConv2d(in_channels, in_channels, 1),
...
class EVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=4, base_channel=16):
        ...
        self.conv1 = ODConv2d(in_channels, in_channels, kernel_size=7, stride=1, padding=3)  # 1 / 2 [112, 112]
        ...
        self.cnv1 = ODConv2d(ch, out_channels, kernel_size=1, stride=1, padding=0)
