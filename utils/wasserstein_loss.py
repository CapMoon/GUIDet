...
def Wasserstein(box1, box2, x1y1x2y2=True):
    box2 = box2.T
    if x1y1x2y2:
        b1_cx, b1_cy = (box1[0] + box1[2]) / 2, (box1[1] + box1[3]) / 2  #中心点坐标 b1_cx, b1_cy
        b1_w, b1_h = box1[2] - box1[0], box1[3] - box1[1]  # 长 宽
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                for id, indexs in dic.items():  # id = batch_name  indexs = target_id
                    if indexs:
                        lrepgt, lrepbox = repulsion_loss_t(pbox[indexs], tbox[i][indexs], deta=deta, pnms=Rp_nms,
                                                           gtnms=Rp_nms)
                        _lrepGT += lrepgt
                        _lrepBox += lrepbox
                        bts += 1
